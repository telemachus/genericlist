...
// Copyright 2022-2023 Peter Aronoff. All rights reserved.
...
func checkListPointers[T any](t *testing.T, l *List[T], es []*Element[T]) {
	...
	for i, e := range es {
		...
		if p := e.prev; p != prev {
			t.Errorf("e[%d](%p).prev = %p, want %p", i, e, p, prev)
		}
		if p := e.Prev(); p != Prev {
			t.Errorf("e[%d](%p).Prev() = %p, want %p", i, e, p, Prev)
		}
		...
		if n := e.next; n != next {
			t.Errorf("et[%d](%p).next = %p, want %p", i, e, n, next)
		}
		if n := e.Next(); n != Next {
			t.Errorf("et[%d](%p).Next() = %p, want %p", i, e, n, Next)
		}
...
func TestList(t *testing.T) {
	...
	for e := l.Front(); e != nil; e = e.Next() {
		if et := reflect.TypeOf(e.Value).Kind(); et == reflect.String {
			sum++
		}
	}
	if sum != l.Len() {
		t.Errorf("sum over l = %d, want %d", sum, l.Len())
	}
...
func checkList(t *testing.T, l *List[int], es []*Element[int]) {
	if !checkListLen(t, l, len(es)) {
		return
	}
	...
	for e := l.Front(); e != nil; e = e.Next() {
		le := e.Value
		if le != es[i].Value {
			t.Errorf("e[%d].Value = %v, want %v", i, le, es[i])
		}
		i++
	}
}
...
func TestExtending(t *testing.T) {
	...
	l3 := New[int]()
	l3.PushBackList(l1)
	checkList(t, l3, []*Element[int]{e1, e2, e3})
	l3.PushBackList(l2)
	checkList(t, l3, []*Element[int]{e1, e2, e3, e4, e5})

	l3 = New[int]()
	l3.PushFrontList(l2)
	checkList(t, l3, []*Element[int]{e4, e5})

	l3.PushFrontList(l1)
	checkList(t, l3, []*Element[int]{e1, e2, e3, e4, e5})

	checkList(t, l1, []*Element[int]{e1, e2, e3})
	checkList(t, l2, []*Element[int]{e4, e5})

	l3 = New[int]()
	l3.PushBackList(l1)
	checkList(t, l3, []*Element[int]{e1, e2, e3})
	l3.PushBackList(l3)
	checkList(t, l3, []*Element[int]{e1, e2, e3, e1, e2, e3})
...
func TestZeroList(t *testing.T) {
	var l1 = new(List[int])
	e1 := l1.PushFront(1)
	checkList(t, l1, []*Element[int]{e1})

	var l2 = new(List[int])
	l2.PushBack(1)
	checkList(t, l2, []*Element[int]{e1})

	var l3 = new(List[int])
	l3.PushFrontList(l1)
	checkList(t, l3, []*Element[int]{e1})

	var l4 = new(List[int])
	l4.PushBackList(l2)
	checkList(t, l4, []*Element[int]{e1})
}
...
func TestInsertBeforeUnknownMark(t *testing.T) {
	...
	checkList(t, &l, []*Element[int]{e1, e2, e3})
}
